...
function LoadGroupGrid() {
    ...
    $grid.jqGrid({
        ...
        datatype: 'json',
        mtype: 'GET',
        colNames: ["id", "GroupId", "Group Name", "Company Name", "No of Users", "Edit", "Delete"],
        colModel: [
        ...
        ondblClickRow: function (rowid, iRow, iCol, event) {
            ...
            if (colname == "GroupName" || colname == "CompanyName") {
                ...
                if (blEdit.toLowerCase() != "false") {
                    ...
                    window.location.href = '../Group/Group?' + rowData.id;
                }
            }
        },
    ...
    // only sysadmin And Admin add update delete Group
    if (hdnUserRoleID != hdnAdminRoleID && hdnUserRoleID != hdnSYSAdminRoleID) {
        jQuery('#tblGroup').jqGrid('hideCol', ['deleteoperation']);
    }
...
function showChildGrid(parentRowID, parentRowKey) {
    ...
    $('#' + childGridID).jqGrid({
        ...
        datatype: "json",
        page: 1,
        colNames: ["GroupId", "Group Name", "Company Name", "User Assigned", "Edit", "Delete"],
        colModel: [
        ...
        subGrid: true,
        subGridRowExpanded: showChildGrid //showThirdChildGrid,
    });
}

//function showThirdChildGrid(parentRowID, parentRowKey) {
//    var childGridID = parentRowID + "_table";
//    var childGridPagerID = parentRowID + "_pager";
//    $('#' + parentRowID).append('<table id=' + childGridID + '></table><div id=' + childGridPagerID + '></div>');
...
function EditFormat(cellvalue, options, rowObject) {
    return "<a href='../Group/Group?" + rowObject.id + "'><label class='IconEdit' title='Edit' alt='' /></a>";
}

function DeleteFormat(cellvalue, options, rowObject) {

    return "<a href='javascript:void(0);' onclick='DeleteItemGroup(\"" + rowObject.id + "\")'><label class='IconDelete' title='Delete' alt='' /></a>";
}
...
function UserChildFormat(cellvalue, options, rowObject) {
    var link = "";
    if (rowObject.UserCount > 0) {
        link = "<a href='javascript:void(0);' title='Click here to view users assigned to Group' onclick='GetUserData(\"" + rowObject.id + "\",\"" + rowObject.GroupName + "\")'><i class='fa fa-info'></i> <span title='Number of usres assigned to group.'>" + rowObject.UserCount + "</span> </a>";
    }
    return link;
}
